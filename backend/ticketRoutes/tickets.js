...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, './routes/uploads');  // Relative path from server root
  },
...
router.get('/tickets/summary', async (req, res) => {
  try {
    ...
    const [[unassigned]] = assignee ? await db.query(queries.unassigned, [assigneeDepartment]) : await db.query(queries.unassigned, params);
...
router.get('/tickets/department/:department', async (req, res) => {
  ...
  try {
    ...
    const query = 'SELECT * FROM ticket WHERE department = ? ORDER BY ticket_created_at DESC';
...
router.get('/tickets/:id', async (req, res) => {
  ...
  try {
    ...
    const [ticketResults] = await db.query('SELECT * FROM ticket WHERE id = ?', [ticketId]);
    if (ticketResults.length === 0) {
      return res.status(404).send('Ticket not found');
    }

    // Fetch attachments associated with the ticket
    const [attachmentsResults] = await db.query('SELECT attachment FROM ticket_attachments WHERE ticket_id = ?', [ticketId]);
...
router.post('/', upload.array('attachments', 10), async (req, res) => {
  ...
  try {
    ...
    if (permissions[0].sendTo.charAt(0) === '1') {
      mails.Admin.forEach((mail) => { allMails.push(mail) });
    }
    if (permissions[0].sendTo.charAt(1) === '1') {
      mails.HOD.forEach((mail) => { allMails.push(mail) });
    }
    if (permissions[0].sendTo.charAt(2) === '1') {
      mails.TicketCreatedBy.forEach((mail) => { allMails.push(mail) });
    }
    if (permissions[0].sendTo.charAt(3) === '1' && mails.AssignedTo) {
      allMails.push(mails.AssignedTo);
    }
    if (permissions[0].sendTo.charAt(4) === '1') {
      mails.Assignees.forEach((mail) => { allMails.push(mail) });
    }
...
router.put('/:ticketId', upload.array('attachments', 10), async (req, res) => {
  ...
  try {
    ...
    const updateTicketQuery = `UPDATE ticket
                               ...
                               WHERE id = ?`;
    await db.query(updateTicketQuery, [title, description, details, department, issue_type, priority, status, assignee, ticketId]);
...
router.put('/tickets/:id/status', async (req, res) => {
  ...
  try {
    const [ticket] = await db.query('SELECT status FROM ticket WHERE id = ?', [ticketId]);
...
router.put('/tickets/:id/assignee', async (req, res) => {
  ...
  try {
    ...
    const [ticket] = await db.query('SELECT assignee FROM ticket WHERE id = ?', [ticketId]);
    ...
    if (results.affectedRows === 0) {
      return res.status(404).json({ message: 'Ticket not found or no change in assignee' });
    }
